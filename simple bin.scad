// bin_with_rounded_corners.scad
// A parametric square bin with rounded corners on both inside and outside.
...
//   convexity         - convexity parameter for preview rendering [1]
...
corner_radius = 10;
floor_thickness = 2;
convexity = 10;       // Safe default for preview [1]
...
module RoundedSquare(width, depth, radius, center = false) {
    // Create a 2D rounded square with given dimensions and corner radius
...
module Bin() {
    difference() {
        ...
        linear_extrude(height = height, convexity = convexity)
            RoundedSquare(outer_width, outer_depth, corner_radius);

        // Inner cavity with rounded corners
        translate([wall_thickness, wall_thickness, floor_thickness])
            linear_extrude(height = inner_height + 1, convexity = convexity)
                RoundedSquare(inner_width, inner_depth, corner_radius);
    }
}
